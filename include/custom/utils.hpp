...
void mouse_callback(GLFWwindow *window, double xpos,
                    double ypos);
void mouse_scroll_callback(GLFWwindow *window, double xpos,
                           double ypos);
...
void rotateScaleObject(GLFWwindow *window) {
  ...
  if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS) {
    ...
    if (axis == 0) {
      modelMat =
          glm::rotate(modelMat, glm::radians(angle), xaxis);
    } else if (axis == 1) {
      modelMat =
          glm::rotate(modelMat, glm::radians(angle), yaxis);
    } else if (axis == 2) {
      modelMat =
          glm::rotate(modelMat, glm::radians(angle), zaxis);
    }
  }
}
void captureScreen(GLFWwindow *window) {
  ...
  if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS) {
    ...
    std::cout << "hit c end" << std::endl;
    std::cout << captureImagePath << std::endl;
  }
}
...
GLuint loadTexture2d(const char *texturePath, GLuint tex,
  ...
  if (data) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, informat, width, height,
...
GLuint loadTexture2d(const char *fpath,
                     bool igamma = false) {
  //
  GLuint tex;
  glGenTextures(1, &tex);
...
void loadHdrTexture(const char *parent, const char *tpath,
                    GLuint &hdrTexture) {
  ...
  float *data = stbi_loadf(fpath.c_str(), &width, &height,
                           &nrComponents, 0);
  if (data) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, width, height,
                 0, GL_RGB, GL_FLOAT,
                 data); // note how we specify the texture's
...
GLuint loadHdrTexture(const char *parent,
                      const char *tpath) {
  // load hdr environment map
  GLuint tex;
...
GLuint loadHdrTexture(int w, int h) {
  ...
  glGenTextures(1, &hdrTexture);
  glBindTexture(GL_TEXTURE_2D, hdrTexture);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, w, h, 0, GL_RG,
               GL_FLOAT, 0);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
...
GLuint loadHdrTexture(int w, int h, GLenum informat,
                      GLenum outformat) {
  // load hdr environment map
  GLuint hdrTexture;
  glGenTextures(1, &hdrTexture);
  glBindTexture(GL_TEXTURE_2D, hdrTexture);
  glTexImage2D(GL_TEXTURE_2D, 0, informat, w, h, 0,
               outformat, GL_FLOAT, 0);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                  GL_REPEAT);
...
GLuint loadTexture2d(const char *texturePath, GLuint tex,
                     unsigned int i, bool isGamma = false) {
  //
...
void saveImage(const char *fpath, GLFWwindow *window) {
  ...
  stbi_write_png(fpath, w, h, nbComponents, buffer.data(),
                 stride);
}
GLuint loadCubeMap(std::vector<fs::path> paths) {
  ...
  for (unsigned int i = 0; i < paths.size(); i++) {
    //
    unsigned char *data =
        stbi_load(paths[i].c_str(), &w, &h, &nbChannels, 0);
    if (data) {
      glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                   GL_RGB, w, h, 0, GL_RGB,
                   GL_UNSIGNED_BYTE, data);
    } else {
      std::cout << "Image not loaded cube side " << i
                << std::endl;
    }
    stbi_image_free(data);
  }
...
void cubeShaderInit_proc_Phong_Phong(Shader myShader) {
  myShader.useProgram();
  float ambientCoeff = 0.1f;
  float shininess = 200.0f;
  glm::vec3 attc(1.0f, 0.0f, 0.0f);
...
GLuint loadA2DTexture() {
  //
  fs::path stonePath =
      textureDirPath / "Stone_001_Diffuse.png";
...
glm::vec4 getNdcFromFrag(glm::vec4 frag, glm::vec4 viewport,
  ...
  ndc.x = nd2.x;
  ndc.y = nd2.y;
  ndc.z = (2.0 * frag.z - nearFar.x - nearFar.y) /
          (nearFar.y - nearFar.x);
  ndc.w = 1.0;
  return ndc;
}

glm::vec4 getClipFromFrag(glm::vec4 frag,
                          glm::vec4 viewport,
                          glm::vec2 nearFar) {
